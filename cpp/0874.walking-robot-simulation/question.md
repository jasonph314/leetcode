# [874. Walking Robot Simulation][link] (Medium)

[link]: https://leetcode.com/problems/walking-robot-simulation/

A robot on an infinite XY-plane starts at point `(0, 0)` facing north. The robot receives an array of
integers `commands`, which represents a sequence of moves that it needs to execute. There are only
...
yᵢ)`. If the robot runs into an obstacle, it will stay in its current location (on the block
